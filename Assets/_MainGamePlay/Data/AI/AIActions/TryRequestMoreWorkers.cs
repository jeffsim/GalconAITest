...
public partial class PlayerAI
{
    ...
    //     private void TrySendWorkersToOwnedNode(AI_NodeState fromNode, ref AIAction bestAction, int curDepth, int thisActionNum)
    //     {
    ...
    // #if DEBUG
    //             var prevEntry = AIDebugger.TrackPerformAction_SendWorkersToOwnedNode(fromNode, toNode, numSent, 0, scoreAfterActionAndBeforeSubActions, debugOutput_ActionsTried++, curDepth);
    // #endif
    //             // Recursively determine what the best action is after this action is performed
    //             var actionScore = DetermineBestActionToPerform(curDepth + 1);
    //             prevEntry.FinalActionScore = actionScore.FinalActionScore;
    //             if (actionScore.FinalActionScore > bestAction.FinalActionScore)
    //             {
    //                 // This is the best action so far in this 'level' of the AI stack; save the action so we can return it
    //                 bestAction.FinalActionScore = actionScore.ThisActionScore;
    ...
    // #if DEBUG
    //                 bestAction.TrackStrategyDebugInfoInAction(actionScore, debugOutput_actionScoreReasons, thisActionNum, curDepth);
    // #endif
    //             }
